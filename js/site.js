...
function renderHome(){
  ...
  document.getElementById("research-grid").innerHTML = siteContent.research.map((r,i)=>`<article class="research-card ${r.featured?"featured":""}"><span>${String(i+1).padStart(2,"0")}</span><h3>${r.title}</h3><p>${r.text}</p></article>`).join("");
  document.getElementById("project-row").innerHTML = siteContent.projects.map((p,i)=>(i?'<div class="project-line"></div>':'')+`<div><p class="project-tag">${p.tag}</p><h3>${p.title}</h3><p>${p.text}</p></div>`).join("");
  renderPublications(document.getElementById("latest-publications"),siteContent.publications.slice(0,3));
  renderNavigation();
}

function personCard(p){
  return `<div class="person"><div class="portrait">${p.photo?`<img src="${p.photo}" alt="${p.name}">`:p.initials}</div><div class="person-name-row"><h3>${p.name}</h3>${p.homepage?`<a class="person-homepage" href="${p.homepage}" target="_blank" rel="noopener">Homepage ↗</a>`:""}</div><p>${p.role}</p>${p.description?`<small>${p.description}</small>`:""}</div>`;
}
...
function newsCard(n){
  const media = n.image ? `<img class="news-image" src="${n.image}" alt="">` : `<div class="news-image news-placeholder"><span>NEWS</span></div>`;
  const link = n.link ? `<a class="news-link" href="${n.link}" target="_blank" rel="noopener">Read more ↗</a>` : "";
  return `<article class="news-card">${media}<div class="news-card-body"><div class="news-date">${formatDate(n.date)}</div><h3>${n.title}</h3><p>${n.text}</p>${link}</div></article>`;
}
